...
// ====== Internal Helpers ======
static void csn(nrf24_t *nrf, bool level) {
    gpio_put(nrf->csn_pin, level);
}
static void ce(nrf24_t *nrf, bool level) {
    gpio_put(nrf->ce_pin, level);
}
...
void nrf24_init(nrf24_t *nrf, spi_inst_t *spi, uint csn_pin, uint ce_pin, bool is_rx) {
    ...
    val = 0x03; nrf_write_reg(nrf, SETUP_RETR, &val, 1);
